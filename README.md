...
### Safe file opening and Unicode rendering

Local files are sampled by their raw content using the selected `--encoding`, not by filename extension. If a direct file looks binary-like or otherwise unlikely to be readable text, interactive htail shows a warning and requires an explicit `y`/`yes` before opening it; pressing Enter declines. Non-interactive runs never wait for confirmation: they warn on stderr and continue with a best-effort decoded view.

Source control characters, including escape sequences and hidden Unicode formatting/separator characters, are displayed visibly rather than sent to the terminal. Legitimate grapheme joiners remain intact for emoji and script shaping. ANSI styling generated by htail remains active, and pane widths and text selection use terminal-cell measurements so accented text, combining marks, CJK, emoji, ANSI styling, and tabs do not shift borders.
